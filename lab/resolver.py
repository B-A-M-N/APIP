...
def _respond(packet: bytes, zone_text: str, baseline_a: str | None) -> bytes:
    tid = packet[:2]
...
def serve(bind: str, port: int, zone_path: str, baseline_a: str | None) -> None:
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    sock.bind((bind, port))
    print(f"lab resolver listening on {bind}:{port} (zone={zone_path}, "
          f"baseline_a={baseline_a})", flush=True)
    while True:
        ...
        try:
            ...
            zone = ""
        try:
            resp = _respond(data, zone, baseline_a)
