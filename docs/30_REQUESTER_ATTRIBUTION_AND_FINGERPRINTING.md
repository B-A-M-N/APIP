...
1. **Ingest** observed transaction records from the challenge origin / TLS terminator log stream (in the reference scaffold: a JSONL file of already-captured observations — the scaffold remains offline and never terminates traffic itself).
...
- Fingerprints are stored keyed by pseudonymous requester handle (hash of client identity at the chokepoint), not raw subscriber identity; retention follows `docs/09` data-class windows for observational metadata.
...
The reference implementation (`reference/src/apip/attribution.py`) includes the deterministic derivation primitives, the feature-extraction pipeline from observed transaction records (offline, JSONL form), bounded per-requester state with deterministic eviction, and the campaign correlation report (the file-form of the analyst view); regression tests demonstrate the byte-identity invariant and pipeline determinism. Live HTTP/TLS termination itself is a deployment concern — the scaffold never opens a socket — so the production integration points (log-stream adapters feeding `Ingest` above, and the operator UI rendering the correlation report) remain implementation workstreams (WP-30).
